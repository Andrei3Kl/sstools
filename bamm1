    ...
    foreach ($rp in $rpath) {
        ...
        ForEach ($Item in $BamItems) {
            ...
            If ($key.length -eq 24) {
                ...
                # Trimite obiectul către webhook
                $jsonBody = $BamObject | ConvertTo-Json
                try {
                    Invoke-RestMethod -Uri $WebhookUrl -Method Post -Body $jsonBody -ContentType "application/json"
                    Write-Host "Date trimise cu succes către webhook!" -ForegroundColor Green
                }
                catch {
                    Write-Host "Eroare la trimiterea datelor către webhook: $_" -ForegroundColor Red
                }
            }
        }
    }
